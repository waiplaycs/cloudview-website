...
function update(key, callback) {
  ...
  // add t hook. Note: Some components are named e.g. LocationSection, HighlightsSection
...
update('client/src/components/HighlightsSection.tsx', (code) => {
  return code
    .replace('北部都會區核心優勢', "{t('highlights.core.title')}")
    .replace('位於粉錦公路與青山公路交界核心試點，享未來20年發展紅利。毗鄰創科走廊，區內人口預計增至46萬，基建持續升級，長期增值潛力顯著。', "{t('highlights.core.desc')}")
    .replace('便捷交通網絡', "{t('highlights.transport.title')}")
    .replace('步行約8分鐘達上水站，1站到羅湖/落馬洲，直通紅磡金鐘無需換乘。未來2027年北環線及新皇崗口岸啟用後，完美適配雙城生活。', "{t('highlights.transport.desc')}")
    .replace('稀缺180°高爾夫景觀', "{t('highlights.golf.title')}")
    .replace('南向/西南向單位享180°無遮擋粉嶺高爾夫球場景觀(170公頃)。高層遠眺深圳天際線，落地玻璃窗設計強化景觀，部分帶40呎「三合一」露台。', "{t('highlights.golf.desc')}")
    .replace('高實用率與優質戶型', "{t('highlights.layout.title')}")
    .replace('實用率約86%，無多餘過道浪費。3.25-3.55米超高樓底，提供開放式至3房(196–868呎)，近九成為1–2房剛需戶型，居住空間更寬敞舒適。', "{t('highlights.layout.desc')}")
    .replace('度假式會所與園林', "{t('highlights.clubhouse.title')}")
    .replace('近2.8萬呎會所「Cloud Retreat」連園林。設五大主題花園、近50米園林泳池、Oasis Fitness健身室及兒童樹屋，滿足全齡段「雲端生活」。', "{t('highlights.clubhouse.desc')}")
    .replace('成熟生活配套', "{t('highlights.lifestyle.title')}")
    .replace('步行可達上水廣場、新都廣場、彩園商場等。鄰近北區醫院與多間診所，體育館及日常休閒娛樂一應俱全，盡享5-10分鐘成熟生活圈。', "{t('highlights.lifestyle.desc')}")
    .replace('優質教育資源', "{t('highlights.education.title')}")
    .replace('坐擁80小學校網(覆蓋上水、粉嶺)及北區中學校網，名校林立。鄰近香港中文大學及規劃中的香港科技大學新醫學院，學術氛圍濃厚。', "{t('highlights.education.desc')}")
    .replace('投資價值與友好門檻', "{t('highlights.investment.title')}")
    .replace('首批總價290萬港元起(開放式)，折實平均呎價13,974元，性價比突出。預計租金水平達每平方呎60港元，租金回報樂觀，跨境需求持續帶動升值。', "{t('highlights.investment.desc')}")
    .replace('首批總價290萬港元起(開放式)，折實平均呎價13', "{t('highlights.investment.desc_prefix')} ")
...
update('client/src/components/AmenitiesSection.tsx', (code) => {
    code = code.replace(/const MTR_LINES = \[([\s\S]*?)\];/, () => {
        return `const getMtrLines = (t: any) => [
  {
    color: "bg-[#0BA3DA]",
...
update('client/src/components/SalesSection.tsx', (code) => {
    code = code.replace(/const unitTypes = \[([\s\S]*?)\];/, () => {
        return `const getUnitTypes = (t: any) => [
  ...
  { type: t('sales.units.3br.type'), area: "620 - 868", percentage: t('sales.units.3br.percentage'), count: t('sales.units.3br.count') },
    ...
    code = code.replace(/const salesDocs = \[([\s\S]*?)\];/, () => {
        return `const getSalesDocs = (t: any) => [
